...
interface ResultCardProps {
  ...
  isFlipped: boolean; // ★この行を追加
  translationDirection: 'en-ja' | 'ja-en'; // ★この行を追加
}

const CardFace = React.forwardRef<HTMLDivElement, {
    searchWord: string; onModeChange: () => void; onRegenerate: () => void;
    content: string; isStreaming: boolean; currentMode: 'quick' | 'deep_dive';
      ...
      <div className="p-4 sm:p-6">
        <div className="flex items-center justify-between pb-4 border-b border-border mb-4 gap-2">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={onModeChange} className="px-4 py-1.5 text-sm font-semibold bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors shadow-md">
...
export default function ResultCard({
  content, isLoading, isStreaming, currentMode, onModeChange, onRegenerate,
  searchWord, onExplainFurther, onCustomQuestion, additionalContent,
  isAdditionalLoading, drillHistory, isFlipped, translationDirection,
}: ResultCardProps) {
  ...
  }, [isFlipped, content, isStreaming, additionalContent, drillHistory]);
      ...
      <div className="perspective" style={{ height: cardHeight, transition: 'height 0.35s ease-in-out' }}>
        <div className={`relative w-full h-full preserve-3d transition-transform duration-700 ease-in-out ${isFlipped ? 'rotate-y-180' : ''}`}>
          <div className={`absolute w-full h-full backface-hidden ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
            <CardFace ref={frontFaceRef} searchWord={searchWord} onModeChange={() => onModeChange()} onRegenerate={onRegenerate} content={content} isStreaming={isStreaming} currentMode="quick" />
          </div>
          <div className={`absolute w-full h-full backface-hidden rotate-y-180 ${isFlipped ? 'opacity-100' : 'opacity-0'}`}>
            <CardFace ref={backFaceRef} searchWord={searchWord} onModeChange={() => onModeChange()} onRegenerate={onRegenerate} content={content} isStreaming={isStreaming} currentMode="deep_dive" />
          </div>
        </div>
      </div>
      {currentMode === 'deep_dive' && content && !isLoading && (
        <DeepDiveFooter onExplainFurther={onExplainFurther} onCustomQuestion={onCustomQuestion} additionalContent={additionalContent} isAdditionalLoading={isAdditionalLoading} drillHistory={drillHistory} />
      )}
    </div>
  );
}